...
class OnBoardingBloc extends Bloc<OnBoardingEvent, OnBoardingState> {
  ...
  OnBoardingBloc({required AppDatasource appDatasource}) :_appDatasource = appDatasource,super(OnBoardingState(onBoardingItems: [],isReachedMaxPage: false)) {
  ...
  _onNavigateTo(OnBoardingNavigateTo event, Emitter<OnBoardingState> emit) {
    if (_isReachedMaxPageCheck(event.pageController)) {
      GoRouter.of(event.context).goNamed(AppRoutes.kAuthScreenRoute);
    }
